...
class PlayerController {
    ...
    _resolveTerrainCollision(dt) {
        ...
        if (this.mesh.position.y < minAllowedY) {
            // push out upward (hard)
        this.mesh.position.y = minAllowedY;

        // ✅ Upward bounce instead of backward knockback
        // gives arcade "terrain bounce" feel
        this.mesh.position.y += 18;

        // ✅ Small forward damp (optional) so it doesn't clip again instantly
        this.mesh.translateZ(10 * dt * 60);

        // ✅ Small sideways drift (prevents stuck)
        this.mesh.position.x += (Math.random() - 0.5) * 8;
        this.mesh.position.z += (Math.random() - 0.5) * 8;

        // ✅ Auto pitch up slightly after collision (feels like pulling up)
        this.mesh.rotation.x = lerp(this.mesh.rotation.x, -0.25, 0.35);
