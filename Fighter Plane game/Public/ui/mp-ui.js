...
class MPUI {
  ...
  injectStyles() {
    ...
    style.innerHTML = `
      :root{ --mp-cyan:#00f3ff; --mp-blue:#0066ff; --mp-bg: rgba(5, 12, 20, 0.82); --mp-card: rgba(10, 25, 40, 0.65); --mp-border: rgba(0, 243, 255, 0.18); --mp-white: rgba(255,255,255,0.92); }
      .mp-overlay{ position: fixed; inset:0; z-index: 3000; display:flex; align-items:center; justify-content:center; background: radial-gradient(circle at center, rgba(10,35,55,0.55) 0%, rgba(0,0,0,0.95) 75%); font-family: Arial, Helvetica, sans-serif; }
      .mp-card{ width: min(560px, 92vw); border: 1px solid var(--mp-border); background: var(--mp-card); backdrop-filter: blur(12px); box-shadow: 0 0 60px rgba(0,0,0,0.55); border-radius: 18px; padding: 22px 22px; color: var(--mp-white); }
      .mp-title{ margin: 0; font-size: 34px; letter-spacing: 2px; font-weight: 900; text-transform: uppercase; color: #fff; text-shadow: 0 0 16px rgba(0,243,255,0.25); }
      .mp-sub{ margin-top: 6px; opacity: 0.8; font-size: 13px; letter-spacing: 1px; }
      .mp-row{ margin-top: 14px; display:flex; gap: 10px; align-items:center; }
      .mp-input{ flex: 1; padding: 12px 12px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.15); background: rgba(0,0,0,0.35); color: #fff; outline: none; font-size: 14px; }
      .mp-input:focus{ border-color: rgba(0,243,255,0.55); box-shadow: 0 0 0 3px rgba(0,243,255,0.12); }
      .mp-btn{ padding: 12px 14px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.18); background: rgba(255,255,255,0.08); color: #fff; cursor:pointer; font-weight: 800; letter-spacing: 0.8px; text-transform: uppercase; transition: transform 0.15s ease, opacity 0.15s ease, background 0.2s; user-select:none; }
      .mp-btn:hover{ transform: translateY(-2px); opacity: 0.96; }
      .mp-btn:active{ transform: translateY(0px); opacity: 0.9; }
      .mp-btn.primary{ background: rgba(0,243,255,0.14); border-color: rgba(0,243,255,0.65); color: var(--mp-cyan); }
      .mp-btn.primary:hover{ background: rgba(0,243,255,0.30); box-shadow: 0 0 18px rgba(0,243,255,0.22); }
      .mp-btn.danger{ background: rgba(255,0,60,0.16); border-color: rgba(255,0,60,0.40); color: rgba(255,220,230,0.95); }
      .mp-btn.danger:hover{ box-shadow: 0 0 18px rgba(255,0,60,0.20); }
      .mp-split{ display:grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 14px; }
      @media (max-width:560px){ .mp-split{ grid-template-columns: 1fr; } }
      .mp-section{ margin-top: 16px; padding-top: 14px; border-top: 1px solid rgba(255,255,255,0.08); }
      .mp-roomcode{ font-family: Consolas, monospace; font-size: 26px; font-weight: 900; letter-spacing: 4px; color: var(--mp-cyan); text-shadow: 0 0 12px rgba(0,243,255,0.25); }
      .mp-players{ margin-top: 10px; display:flex; flex-direction:column; gap: 8px; }
      .mp-player{ display:flex; justify-content:space-between; gap: 10px; padding: 10px 12px; border-radius: 12px; background: rgba(0,0,0,0.25); border: 1px solid rgba(255,255,255,0.08); font-size: 14px; }
      .mp-badge{ font-size: 11px; font-weight: 900; letter-spacing: 1px; padding: 4px 9px; border-radius: 999px; border: 1px solid rgba(0,243,255,0.25); background: rgba(0,243,255,0.10); color: var(--mp-cyan); text-transform: uppercase; }
      .mp-msg{ margin-top: 10px; font-size: 13px; opacity: 0.85; }
      .mp-error{ margin-top: 12px; font-size: 13px; color: rgba(255,140,140,0.95); font-weight: 700; }
      .mp-hud{ position: fixed; top: 12px; left: 12px; z-index: 200; font-family: Consolas, monospace; font-size: 13px; color: rgba(255,255,255,0.8); background: rgba(0,0,0,0.35); border: 1px solid rgba(255,255,255,0.10); padding: 10px 12px; border-radius: 12px; pointer-events: none; display:none; }
    `;
  ...
  updateLobby(players = [], hostId = null, myId = null) {
    ...
    this.playersBox.innerHTML = players.map(p => `
      <div class="mp-player">
        <div>${p.name || "Pilot"}</div>
        <div>${(p.id === hostId) ? `<span class="mp-badge">HOST</span>` : ""}</div>
      </div>
    `).join("");
    this._refreshLobbyButtons();
  }
...
function unlockAudio() {
    if (window.game && window.game.procAudio) {
        ...
        if (window.game.procAudio.musicPlaying !== "lobby" && window.game.procAudio.musicPlaying !== "game") {
...
function sanitizeCode(v) { return (v || "").toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 4); }
if (inpCode) inpCode.addEventListener("input", () => { inpCode.value = sanitizeCode(inpCode.value); });
...
if (btnLeave) btnLeave.onclick = () => window.location.reload();
